...
class Tabuleiro
	attr_accessor :width, :height, :num_mines, :tabuleiro
		...
		(0...@height).each do |i|
			(0...@width).each do|j|
				...
				if array_minas.find_index(aux)
					@tabuleiro.[]=(i, j, Celula.new($mine))
				else
					@tabuleiro.[]=(i, j, Celula.new("0"))
				end
			end
		end
	...
	def busca_por_celulas_vizinhas_sem_mina(x, y)
		if x >= 0 && y >= 0 && x <= @height - 1 && y <= @width - 1
			if @tabuleiro[x, y].valor == ' ' && !@tabuleiro[x, y].descoberta? && !@tabuleiro[x, y].flag?
                descobre_celulas_vizinhas_sem_mina(x, y)
            elsif !@tabuleiro[x, y].mine? && !@tabuleiro[x, y].descoberta? && !@tabuleiro[x, y].flag?
    ...
    def descobre_celulas_vizinhas_sem_mina(x, y)
		...
		ay = [-1, 0, 1, -1, 1, -1, 0, 1]
		(0...8).each do |i| 
			busca_por_celulas_vizinhas_sem_mina(x + ax[i], y + ay[i])
		end
    end
...
class Minesweeper
	attr_accessor :still_playing, :victory, :tabuleiro
	def initialize(width, height, num_mines)
        @tabuleiro = Tabuleiro.new(width, height, num_mines)
        @still_playing = true
        @victory = false
	end
		
	def preenche_tabuleiro
		@tabuleiro.preenche_tabuleiro
	end
	def still_playing?
        count = 0
        pisou = false
        (0...@tabuleiro.height).each do |i|
			(0...@tabuleiro.width).each do|j|
                #conta se o número de células cobertas bate com o número de minas
				if !@tabuleiro.[](i, j).descoberta?
					count += 1
				end
				#verifica se pisou em uma mina
				if @tabuleiro.[](i, j).descoberta? && @tabuleiro.[](i, j).mine?
					pisou = true
				end
			end
		end
		#condição de vitória
		if count == @tabuleiro.num_mines && !pisou
		    @victory = true
		    @still_playing = false
        #mid-game
        elsif count > @tabuleiro.num_mines && !pisou
            @victory = false
            @still_playing = true
        #condição de derrota
        elsif count > @tabuleiro.num_mines && pisou
    ...
    def play(x, y)
        ...
        #Jogada válida
        if @tabuleiro.[](y, x).mine? #Em uma mina
            @still_playing = false
            @victory = false
            ...
            @tabuleiro.descobre_celulas_vizinhas_sem_mina(y, x)
            return true
        end
    ...
    def board_state(xray = false)
        tab = Matrix.build(@tabuleiro.height,@tabuleiro.width){0}
		(0...@tabuleiro.height).each do |i|
            (0...@tabuleiro.width).each do|j|
        ...
        if xray
            (0...@tabuleiro.height).each do |i|
                (0...@tabuleiro.width).each do|j|
                    if !tab[i, j].descoberta? && !tab[i, j].mine?
                        tab[i, j].valor = '.'
                    end
                    if tab[i, j].flag? && !tab[i, j].mine?
                        tab[i, j].valor = 'F'
                    end
                end
            end
